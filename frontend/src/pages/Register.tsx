...
export default function Register() {
  ...
  const password = watch('құпия_сөз');

  const onSubmit = async (data: RegisterForm) => {
    setLoading(true);
    setError('');
    try {
      const response = await authService.register(data.аты, data.эл_пошта, data.құпия_сөз);
    ...
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-white to-blue-50 flex items-center justify-center p-4">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-500 mt-1">Жаңа тіркелгі жасаңыз</p>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-xl p-8 border border-gray-100">
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1.5">Аты-жөні</label>
              <input
                type="text"
                {...register('аты', { required: 'Атыңызды енгізіңіз', minLength: { value: 2, message: 'Ат кемінде 2 символ болуы керек' } })}
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1.5">Электрондық пошта</label>
              <input
                type="email"
                {...register('эл_пошта', {
            ...
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  {...register('құпия_сөз', {
                    ...
                    minLength: { value: 6, message: 'Құпия сөз кемінде 6 символ болуы керек' },
                  })}
              ...
              {errors.құпия_сөз && <p className="mt-1 text-xs text-red-600">{errors.құпия_сөз.message}</p>}
            </div>

            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1.5">Құпия сөзді растау</label>
              <input
                type="password"
                {...register('растау_сөз', {
                  required: 'Құпия сөзді растаңыз',
                  validate: (val) => val === password || 'Құпия сөздер сәйкес келмейді',
                })}
                className="w-full rounded-lg border border-gray-300 px-4 py-2.5 text-gray-900 placeholder-gray-400 focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500"
                placeholder="••••••••"
              />
              {errors.растау_сөз && <p className="mt-1 text-xs text-red-600">{errors.растау_сөз.message}</p>}
            </div>
